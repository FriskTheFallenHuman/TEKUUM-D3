...
void idImage::ActuallyLoadImage( bool fromBackEnd )
{
	...
	else
	{
		...
		if( cubeFiles == CF_NATIVE || cubeFiles == CF_CAMERA || cubeFiles == CF_SINGLE )
		{
			...
			sessLocal.PacifierBinarizeFilename( generatedName.c_str(), binarizeReason.c_str() );
			if( opts.numLevels > 1 )
			{
				sessLocal.PacifierBinarizeProgressTotal( opts.width * opts.width * 6 * 4 / 3 );
			}
			else
			{
				sessLocal.PacifierBinarizeProgressTotal( opts.width * opts.width * 6 );
			}

			im.LoadCubeFromMemory( size, ( const byte** )pics, opts.numLevels, opts.format, opts.gammaMips );

			sessLocal.PacifierBinarizeEnd();
...
void idImage::GenerateImage( const byte* pic, int width, int height, textureFilter_t filterParm, textureRepeat_t repeatParm, textureUsage_t usageParm, textureSamples_t samples, cubeFiles_t _cubeFiles, bool isRenderTarget )
{
	...
	else
	{
		...
		// foresthale 2014-05-30: give a nice progress display when binarizing
		sessLocal.PacifierBinarizeFilename( GetName() , "generated image" );
		if( opts.numLevels > 1 )
		{
			sessLocal.PacifierBinarizeProgressTotal( opts.width * opts.height * 4 / 3 );
		}
		else
		{
			sessLocal.PacifierBinarizeProgressTotal( opts.width * opts.height );
		}
...
void idImage::GenerateCubeImage( const byte* pic[6], int size, textureFilter_t filterParm, textureUsage_t usageParm )
{
	...
	// foresthale 2014-05-30: give a nice progress display when binarizing
	sessLocal.PacifierBinarizeFilename( GetName(), "generated cube image" );
	if( opts.numLevels > 1 )
	{
		sessLocal.PacifierBinarizeProgressTotal( opts.width * opts.width * 6 * 4 / 3 );
	}
