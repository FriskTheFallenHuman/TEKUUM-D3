...
void idBinaryImage::Load2DFromMemory( int width, int height, const byte* pic_const, int numLevels, textureFormat_t& textureFormat, textureColor_t& colorFormat, bool gammaMips )
{
	...
	sessLocal.PacifierBinarizeInfo( va( "(%d x %d)", width, height ) );
	...
	for( int level = 0; level < images.Num(); level++ )
	{
		...
		sessLocal.PacifierBinarizeMiplevel( level + 1, numLevels );
		...
		if( textureFormat == FMT_DXT1 )
		{
			...
			if( image_highQualityCompression.GetBool() )
			{
				sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT1HQ", width, height ) );

				dxt.CompressImageDXT1HQ( dxtPic, img.data, dxtWidth, dxtHeight );
			}
			else
			{
				sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT1Fast", width, height ) );
		...
		else if( textureFormat == FMT_DXT5 )
		{
			...
			if( colorFormat == CFM_NORMAL_DXT5 )
			{
				if( image_highQualityCompression.GetBool() )
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - NormalMapDXT5HQ", width, height ) );

					dxt.CompressNormalMapDXT5HQ( dxtPic, img.data, dxtWidth, dxtHeight );
				}
				else
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - NormalMapDXT5Fast", width, height ) );
			...
			else if( colorFormat == CFM_YCOCG_DXT5 )
			{
				if( image_highQualityCompression.GetBool() )
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - YCoCgDXT5HQ", width, height ) );

					dxt.CompressYCoCgDXT5HQ( dxtPic, img.data, dxtWidth, dxtHeight );
				}
				else
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - YCoCgDXT5Fast", width, height ) );
			...
			else
			{
				...
				if( image_highQualityCompression.GetBool() )
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT5HQ", width, height ) );

					dxt.CompressImageDXT5HQ( dxtPic, img.data, dxtWidth, dxtHeight );
				}
				else
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT5Fast", width, height ) );
...
void idBinaryImage::Load2DAtlasMipchainFromMemory( int width, int height, const byte* pic_const, int numLevels, textureFormat_t& textureFormat, textureColor_t& colorFormat )
{
	...
	byte* sourcePic = ( byte* )Mem_Alloc( width * height * 4 );
	...
	for( int level = 0; level < images.Num(); level++ )
	{
		...
		byte* pic = ( byte* )Mem_Alloc( scaledWidth * scaledHeight * 4 );
...
{
	sessLocal.PacifierBinarizeInfo( va( "cube (%d)", width ) );
	...
	for( int side = 0; side < 6; side++ )
	{
		...
		for( int level = 0; level < fileData.numLevels; level++ )
		{
			...
			sessLocal.PacifierBinarizeMiplevel( level, fileData.numLevels );
			...
			if( textureFormat == FMT_DXT1 )
			{
				...
				if( image_highQualityCompression.GetBool() )
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT1HQ", width, width ) );

					dxt.CompressImageDXT1HQ( padSrc, img.data, padSize, padSize );
				}
				else
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT1Fast", width, width ) );
			...
			else if( textureFormat == FMT_DXT5 )
			{
				...
				if( image_highQualityCompression.GetBool() )
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT5HQ", width, width ) );

					dxt.CompressImageDXT5HQ( padSrc, img.data, padSize, padSize );
				}
				else
				{
					sessLocal.PacifierBinarizeInfo( va( "(%d x %d) - DXT5Fast", width, width ) );
...
bool idBinaryImage::LoadFromGeneratedFile( idFile* bFile, ID_TIME_T sourceTimeStamp )
{
	...
	if( ( sourceTimeStamp != FILE_NOT_FOUND_TIMESTAMP ) && ( sourceTimeStamp != 0 ) && ( sourceTimeStamp != fileData.sourceFileTime ) )
	{
		return false;
	}
	// RB end
