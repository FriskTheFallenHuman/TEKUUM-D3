...
// SRS - Generalized Vulkan SDL platform
	...
	//typedef uint32_t QueryTuple[2];

	// GPU timestamp queries
	idArray< uint32_t, NUM_FRAME_DATA >									queryIndex;
	idArray< idArray< uint32_t, MRB_TOTAL_QUERIES >, NUM_FRAME_DATA >		queryAssignedIndex;
	idArray< idArray< uint64_t, NUM_TIMESTAMP_QUERIES >, NUM_FRAME_DATA >	queryResults;
...
struct glContext_t
{
	...
	uint64_t		stencilOperations[ STENCIL_FACE_NUM ];

	// for GL_TIME_ELAPSED_EXT queries
	GLuint		renderLogMainBlockTimeQueryIds[ NUM_FRAME_DATA ][ MRB_TOTAL_QUERIES ];
	uint32_t		renderLogMainBlockTimeQueryIssued[ NUM_FRAME_DATA ][ MRB_TOTAL_QUERIES ];
};
