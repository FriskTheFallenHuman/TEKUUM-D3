...
void setGLMode( int mode )
{
	switch( mode )
	{
		case cd_wire:
			...
			tr.backend.GL_Color( 1.0f, 1.0f, 1.0f );
			renderProgManager.Unbind();
			break;
...
void CCamWnd::Cam_Draw()
{
	...
	tr.backend.GL_Color( colorWhite );
	...
	tr.backend.GL_Color( g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][0], g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][1], g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][2], 0.25f );
...
void CCamWnd::BuildRendererState()
{
	...
	worldModel->FinishSurfaces( false );
